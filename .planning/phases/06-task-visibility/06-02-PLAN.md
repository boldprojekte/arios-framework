...
3. In startServer:
   ...
   - Pass broadcast function as onStateChange callback:
     ```typescript
     // Ensure callback type matches: (state: DashboardState) => void
     const watcher = createWatcher(planningDir, (state: DashboardState) => {
...
**Type alignment verification:** Ensure createWatcher's onStateChange callback type `(state: DashboardState) => void` matches the usage in startServer where it wraps the state in an SSEMessage before calling broadcast. The watcher callback receives DashboardState directly, and the server wraps it for SSE transmission.
  </action>
  <verify>
