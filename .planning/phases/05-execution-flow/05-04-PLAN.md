...
must_haves:
  ...
  key_links:
    - from: "recovery.ts"
      ...
      pattern: "import.*CheckpointResult.*from.*checkpoint"
    - from: "recovery.ts"
...
3. `spawnDebugSubagent(cwd: string, failure: CheckpointResult): Promise<{diagnosis: string, planPath: string}>` (stub):
   ...
   - Real implementation will use Task tool to spawn debug agent
...
4. `executeDebugPlan(planPath: string): Promise<{success: boolean}>` (stub):
   ...
   - Real implementation will invoke executor on the debug plan
...
- Stub functions clearly marked for future implementation
</success_criteria>
