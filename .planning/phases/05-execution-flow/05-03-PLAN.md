...
Note: The actual selection UI prompt (offering the user the choice) is implemented by the orchestrator's /ideate command, not by this module. This module provides the storage and retrieval API that the orchestrator uses.
</objective>
...
7. `hasApproachSet(projectDir: string): Promise<boolean>`:
   ...
   - Used by orchestrator to determine if selection prompt should be shown at project start
...
Per CONTEXT.md: "Offered at project start only" - hasApproachSet enables orchestrator to check this.
