...
Note: If 05-01 hasn't run yet, create the file with just these types. The types will be merged when both plans complete.

Follow existing type patterns with JSDoc comments.
  </action>
  <verify>npx tsc --noEmit packages/arios-cli/src/types/execution.ts</verify>
  <done>CheckpointConfig and CheckpointResult types compile and export</done>
</task>
