...
must_haves:
  truths:
    ...
    - "Feature-Mode uses same state schema as Project-Mode (full tracking)"
  artifacts:
...
# STATE-03 clarification: Per CONTEXT.md, "Feature-Mode gets full tracking
# (same as Project-Mode, minus roadmap/phases)". This means "lighter" refers
# to folder isolation and no phase numbering, NOT a different state schema.
# The STATE.md format is identical; the difference is:
# - Project-Mode: .planning/STATE.md + .planning/phases/XX-name/
# - Feature-Mode: .planning/features/feature-{name}/STATE.md (no phases folder)
---

<objective>
Implement Feature-Mode folder structure with separate state paths and archive behavior.

Purpose: Feature-Mode work should be isolated in .planning/features/feature-{name}/ with its own STATE.md, and archive to .planning/archive/ on completion. Feature-Mode uses the same state schema as Project-Mode — the "lighter" aspect is folder isolation without roadmap/phases structure, not a different schema.
...
- Both modes use same state/plan schema (only folder structure differs)
</verification>
...
- STATE-03 satisfied: "lighter" = folder isolation, not schema differences
</success_criteria>
