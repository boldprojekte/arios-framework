...
Purpose: Enable session continuity by persisting project state in .planning/STATE.md with YAML frontmatter (matching existing ARIOS handoff patterns). These TypeScript utilities are for PROGRAMMATIC use by ARIOS orchestrator code (like execute-phase, verify-phase) - NOT for slash commands, which read STATE.md directly.

Output: TypeScript types and utility functions for state management that ARIOS orchestrator workflows can import.
</objective>
...
These utilities are for ORCHESTRATOR CODE (execute-phase, verify-phase, etc.) - NOT for slash commands.
