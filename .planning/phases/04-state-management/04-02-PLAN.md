...
**CRITICAL PATH MIGRATION:** Replace ALL `.arios/` references with `.planning/` references throughout the file. This includes:
- Directory checks: `.arios/` -> `.planning/`
- File references: `.arios/STATE.md` -> `.planning/STATE.md`
- Config references: `.arios/config.json` -> `.planning/config.json`
- Instructions and comments mentioning `.arios/`
...
- Has @.planning/STATE.md in Context section (NOT .arios/STATE.md)
- `grep '.arios' packages/arios-cli/templates/.claude/commands/arios/arios.md` returns NO matches
...
- Preserves existing greenfield/brownfield logic
  ...
  <done>
/arios command instructs Claude to read .planning/STATE.md directly, parse frontmatter, display mini status table, handle conflicts collaboratively, and offer non-assertive suggestions. No `.arios/` references remain.
  </done>
</task>
...
- Has @.planning/STATE.md in Context section (NOT .arios/STATE.md)
- `grep '.arios' packages/arios-cli/templates/.claude/commands/arios/status.md` returns NO matches
...
- Non-assertive suggestion at end
  ...
  <done>
/arios:status instructs Claude to read .planning/STATE.md directly, display mini table with phase/plan position, show recent decisions including rejected ideas, and offer non-assertive continuation suggestion. No `.arios/` references remain.
  </done>
</task>
...
7. **No `.arios/` references remain in either file** - verify with `grep '.arios' <file>` returning empty
</verification>
...
- **Path migration complete:** All references use `.planning/` not `.arios/`
</success_criteria>
