...
Expected checksum value: `4ce5a813`
...
Use Node.js one-liner to validate (must run from packages/arios-cli where gray-matter is installed):
```bash
cd packages/arios-cli && node -e "
const fs = require('fs');
...
This runs from packages/arios-cli directory where gray-matter is available as a dependency.
  </action>
  <verify>
