...
5. If execution needed:
   ...
   - **Extract codebase patterns before spawning wave-executor**
   ...
   - **Execute wave by wave:**
     a. For each wave in order:
        - If wave has multiple plans: spawn wave-executor for each plan in parallel (concurrent Tasks)
        - If wave has single plan: spawn wave-executor sequentially
        - Wait for all wave-executors in wave to complete before starting next wave
   ...
   - Spawn wave-executor(s) with plan path, wave number, task IDs, problems path, **patterns path**
...
Additional context:
- Wave number: {N}
- Patterns file: .planning/patterns.json (wave-executor reads this itself)
